...
var zimbra_notifier_SERVICE_EVENT = {
    ...
    TWOFA_AUTHENTICATION_REQUIRED: {
        startingReq: true,
        n: 'TWOFA_AUTHENTICATION_REQUIRED',
    },
...
zimbra_notifier_Service.prototype._getWebService = function () {
    if (!this._webservice) {
        ...
        if (wSet !== null) {
            this._webservice.restoreWaitSet(
                this._parent._accountId,
                wSet.id,
                wSet.seq,
                wSet.urlWebService,
                wSet.user,
...
zimbra_notifier_Service.prototype._planRunState = function (newState, delayMs) {
    var object = this;
    this._stateTimer = zimbra_notifier_Util.setTimer(
        this._stateTimer,
        function () {
...
zimbra_notifier_Service.prototype._runState = function (newState) {
    switch (newState) {
        ...
        case zimbra_notifier_SERVICE_STATE.CONNECT_WAIT:
            this._delayWaitConnect +=
                zimbra_notifier_Constant.SERVICE.CONNECT_BASE_WAIT_AFTER_FAILURE;
            if (
                this._delayWaitConnect >
                zimbra_notifier_Constant.SERVICE.CONNECT_MAX_WAIT_AFTER_FAILURE
            ) {
                this._delayWaitConnect =
                    zimbra_notifier_Constant.SERVICE.CONNECT_MAX_WAIT_AFTER_FAILURE;
            }
        ...
        case zimbra_notifier_SERVICE_STATE.CALENDAR_RUN:
            if (
                zimbra_notifier_Prefs.isCalendarEnabled() &&
        ...
        case zimbra_notifier_SERVICE_STATE.TASK_RUN:
            if (
                zimbra_notifier_Prefs.isTaskEnabled() &&
        ...
        case zimbra_notifier_SERVICE_STATE.REFRESH_ENDED:
            ...
            if (runagain === true) {
                this._planRunState(
                    zimbra_notifier_SERVICE_STATE.UNREAD_MSG_RUN,
        ...
        case zimbra_notifier_SERVICE_STATE.WAITSET_NO_NEW_EVT:
            if (this._timeStartWaitReq === 0) {
                ...
                var timeMinW =
                    this._timeStartWaitReq + zimbra_notifier_Constant.SERVICE.WAITSET_MIN_DURATION;
                if (timeMinW > timeEndW) {
                    delayAfterWaitReq = timeMinW - timeEndW;
                }
                // Check if we need to run again a blocking wait set
                var timeEndLoop =
                    this._timeStartLoopWaitReq +
                    zimbra_notifier_Prefs.getRequestWaitLoopTime(this._parent._accountId);
                if (timeEndLoop > timeEndW) {
                    ...
                    if (this._needCheckAgainWaitSet === true) {
                        ...
                        this._planRunState(
                            zimbra_notifier_SERVICE_STATE.WAITSET_BLOCK_RUN,
                            delayAfterWaitReq,
                        );
                    }
                } else {
                    // End of loop
                    this._planRunState(
                        zimbra_notifier_SERVICE_STATE.CONNECT_CHECK,
                        delayAfterWaitReq,
                    );
                }
            }
...
zimbra_notifier_Service.prototype._doSearchCalendar = function () {
    if (this._checkExpectedState(zimbra_notifier_SERVICE_STATE.CALENDAR_RUN)) {
        ...
        var endDate = new Date(
            dateN + 86400000 * zimbra_notifier_Prefs.getCalendarPeriodDisplayed(),
        );
        var startDate = new Date(
            dateN - zimbra_notifier_Constant.SERVICE.EVENTS_DELTA_START_FROM_NOW,
        );
...
zimbra_notifier_Service.prototype.prefUpdated = function (connecting) {
    ...
    if (!connecting) {
        if (
            zimbra_notifier_Prefs.isAutoConnectEnabled() &&
            zimbra_notifier_Prefs.isSavePasswordEnabled(this._parent.accountId) &&
            !this.isConnected() &&
            this._currentState !== zimbra_notifier_SERVICE_STATE.CONNECT_RUN
        ) {
...
zimbra_notifier_Service.prototype.callbackError = function (typeReq, statusReq) {
    ...
    if (
        statusReq === zimbra_notifier_REQUEST_STATUS.AUTH_REQUIRED &&
    ...
    switch (typeReq) {
        ...
        case zimbra_notifier_REQUEST_TYPE.WAIT_BLOCK:
            ...
            if (
                this._timeStartWaitReq < 1000 ||
                new Date().getTime() <
                    zimbra_notifier_Constant.SERVICE.WAITSET_MIN_DURATION +
                        this._timeStartWaitReq -
                        500
            ) {
                this._reqInfoErrors.addError(typeReq, statusReq);
            } else {
        ...
        case zimbra_notifier_REQUEST_TYPE.WAIT_NO_BLOCK:
            ...
            if (
                this._checkExpectedStates([
                    zimbra_notifier_SERVICE_STATE.WAITSET_CHECK_RUN,
                    zimbra_notifier_SERVICE_STATE.WAITSET_NO_BLOCK_RUN,
                ])
            ) {
...
zimbra_notifier_Service.prototype.callbackLoginSuccess = function (twoFactorAuthRequired) {
    ...
    if (
        this._checkExpectedState(zimbra_notifier_SERVICE_STATE.CONNECT_RUN) &&
 ...
 */
zimbra_notifier_Service.prototype.callbackSessionInfoChanged = function (
    session,
    needSaveDevicetrustedInfos = false,
) {
    zimbra_notifier_Prefs.saveWaitSet(
        this._parent._accountId,
        session.waitId(),
        session.waitSeq(),
        session.buildUrl(''),
    ...
    if (needSaveDevicetrustedInfos) {
        zimbra_notifier_Prefs.saveDeviceTrustedInfos(
            this._parent._accountId,
            session.deviceId(),
            session.trustedToken(),
            session.trustedTokenExpirationTime(),
        );
    }
    ...
    try {
        ...
        if (notify && nbNewMsg > 0 && zimbra_notifier_Prefs.isEmailNotificationEnabled()) {
            ...
            if (nbNewMsg > 1 || !lastSender) {
                title = zimbra_notifier_Util.getBundleString(
                    'connector.notification.nbUnreadMessages',
                );
            ...
            for (
                var idx = 0;
                idx < listNewSubject.length &&
                idx < zimbra_notifier_Constant.SERVICE.NOTIFY_MAX_NB_MSG;
                ++idx
            ) {
                msgTxt +=
                    '\n' +
                    zimbra_notifier_Util.maxStringLength(
                        listNewSubject[idx],
                        zimbra_notifier_Constant.SERVICE.NOTIFY_MAX_LEN_TITLE,
                    ) +
                    '\n';
            }
            ...
            zimbra_notifier_Util.showNotification(
                title,
                msgTxt,
                zimbra_notifier_Prefs.getEmailNotificationDuration(),
                browser.openWebPage,
                browser,
            );
        }
        ...
        if (nextOffset > 0) {
            if (currOffset !== this._unreadMessageOffset) {
                this._logger.warning(
                    'Unexpected unread msg query offset, got: ' +
                        currOffset +
                        ' expecting: ' +
                        this._unreadMessageOffset,
                );
            }
        ...
        for (index = 0; index < events.length; index++) {
            ...
            for (indexC = 0; indexC < this._currentEvents.length; indexC++) {
                ...
                if (oldEvent.id === newEvent.id && oldEvent.notifier) {
                    ...
                    // refresh notifier
                    newEvent.notifier.update(
                        newEvent,
                        zimbra_notifier_Prefs.getCalendarReminderTimeConf(),
