...
zimbra_notifier_Webservice.prototype.authRequest = function (urlWebService, login, password) {
    ...
    this._launchQuery(typeReq, false, false, function () {
        ...
        this._runningReq = this._buildQueryReq(
            typeReq,
            '/service/soap/AuthRequest',
            this._callbackAuthRequest,
        );
        this._runningReq.setAuthRequest(
            login,
            password,
            this._session.deviceId(),
            this._session.trustedToken(),
        );
        return true;
    });
};
        ...
        if (request.isSuccess()) {
            ...
            if (
                ...
                this._session.updateToken(
                    jsonResponse.Body.AuthResponse.authToken[0]._content,
                    jsonResponse.Body.AuthResponse.lifetime,
                    !!jsonResponse.Body.AuthResponse.twoFactorAuthRequired,
                    jsonResponse.Body.AuthResponse.deviceId?._content,
                    jsonResponse.Body.AuthResponse.trustedToken?._content,
                    jsonResponse.Body.AuthResponse.trustLifetime?._content,
                );
        ...
        if (request.isSuccess()) {
            ...
            if (jsonR && jsonR.Body && jsonR.Body.CreateWaitSetResponse) {
                this._session.updateWaitSet(
                    jsonR.Body.CreateWaitSetResponse.waitSet,
...
zimbra_notifier_Webservice.prototype.waitRequest = function (blocking) {
    var typeReq = blocking
        ? zimbra_notifier_REQUEST_TYPE.WAIT_BLOCK
    ...
    try {
        ...
        if (request.isSuccess()) {
            ...
            if (jsonR && jsonR.Body && jsonR.Body.WaitSetResponse) {
                ...
                this._session.updateWaitSet(
                    jsonR.Body.WaitSetResponse.waitSet,
            ...
            if (jsonResponse && jsonResponse.Body) {
                ...
                if (content) {
                    for (var iMsgId = 0; iMsgId < content.length; ++iMsgId) {
                        messages.push(
                            new zimbra_notifier_Message(content[iMsgId].id, 0, '', '', 0, null),
                        );
                    }
                }
                ...
                if (content) {
                    for (var index = content.length - 1; index >= 0; index--) {
                        if (content[index].inst && content[index].inst.length > 0) {
                            ...
                            var task = new zimbra_notifier_Task(
                                currTask.name,
                                currTask.d,
...
zimbra_notifier_Webservice.prototype._buildQueryReq = function (typeReq, url, callback) {
    return new zimbra_notifier_Request(
        typeReq,
        this._timeoutQuery,
        this._session.buildUrl(url),
        this,
        callback,
    );
};
 ...
 */
zimbra_notifier_Webservice.prototype._launchQuery = function (
    typeReq,
    connectRequired,
    waitSetRequired,
    func,
...
zimbra_notifier_Webservice.prototype._runCallbackFailLaunch = function (typeReq, status) {
    var object = this;
    this._timerLaunchCallback = zimbra_notifier_Util.setTimer(
        this._timerLaunchCallback,
        function () {
