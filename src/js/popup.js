...
zimbra_notifier_popup.init = function () {
    ...
    $('#zimbra_mail_notifier_tooltipOption').on(
        'click',
        $.proxy(function () {
            this.optionClick();
        }, this),
    ...
    $('#zimbra_mail_notifier_tooltipCheckNow').on(
        'click',
        $.proxy(function () {
            this.checkNowClick();
        }, this),
...
zimbra_notifier_popup.initializeTooltipIdentifier = function (controller) {
    ...
    if (controller.isConnected) {
        $('#zimbra_mail_notifier_tooltipIdentifier_' + accountId).append(
            '<div id="zimbra_mail_notifier_tooltipTitle_' +
                accountId +
                '" class="tooltipTitle"></div><div id="zimbra_mail_notifier_tooltipMessage_' +
                accountId +
                '"  class="tooltipMessage"></div><div class="action actionidentifier" ><img id="zimbra_mail_notifier_tooltipHome_' +
                accountId +
                '" src="skin/images/button_home.png" msgTitle="main_homepage_label"/><img id="zimbra_mail_notifier_tooltipDisconnect_' +
                accountId +
                '" src="skin/images/button_disconnect.png" msgTitle="main_disconnect" /></div>',
        );
        $('#zimbra_mail_notifier_tooltipHome_' + accountId).on('click', function () {
            zimbra_notifier_popup
                .driver('openZimbraWebInterface', controller.accountId)
                .then(() => {
                    window.close();
                });
        });
        ...
        $('<div/>', {
            text: chrome.i18n
                .getMessage('tooltip_connected_descriptionAccount')
                .replace(
                    '%EMAIL%',
                    zimbra_notifier_popup._zimbra_notifier_Prefs.getUserAlias(accountId),
                ),
        ...
        $('#zimbra_mail_notifier_tooltipIdentifier_' + accountId).append(
            '<div id="zimbra_mail_notifier_tooltipTitle_' +
                ...
                '" class="tooltipTitle"></div><div id="zimbra_mail_notifier_tooltipMessage_' +
                accountId +
                '" class="tooltipMessage"></div><div class="action actionidentifier">' +
                (controller.isConnecting && controller.needTwoFactorAuth
                    ? '<img id="zimbra_mail_notifier_tooltipDisconnect_' +
                      accountId +
                      '" src="skin/images/button_disconnect.png" msgTitle="main_disconnect" />'
                    : '<img id="zimbra_mail_notifier_tooltipConnect_' +
                      accountId +
                      '" src="skin/images/button_connect.png" msgTitle="main_connect" />') +
                '</div>',
        );
        $('#zimbra_mail_notifier_tooltipDisconnect_' + accountId).on('click', function () {
            zimbra_notifier_popup.driver('closeConnection', controller.id);
        ...
        $('#zimbra_mail_notifier_tooltipConnect_' + accountId).on('click', function () {
            if (!zimbra_notifier_popup._zimbra_notifier_Prefs.isSavePasswordEnabled(accountId)) {
                zimbra_notifier_popup.openOptionPage(
                    zimbra_notifier_UiUtil.OPTION_SELECT_TAB.IDENTIFICATION,
                );
                return;
            }
        ...
        if (controller.isConnecting && controller.needTwoFactorAuth) {
            ...
            $(
                '<div> \
                <div>' +
                    chrome.i18n.getMessage('option_identifiant_2fatoken_label') +
                    ' :</div> \
                <input id="zimbra_mail_notifier-option2faToken' +
                    accountId +
                    '" type="text" style="width:100px;margin-right:4px;"></input> \
                <button id="zimbra_mail_notifier-2faTokenButton' +
                    accountId +
                    '">' +
                    chrome.i18n.getMessage('option_identifiant_2fatoken_button') +
                    '</button> \
            </div>',
            ).appendTo('#zimbra_mail_notifier_tooltipMessage_' + accountId);
    ...
    await Promise.all(
        controllers.map(function (controller) {
            ...
            if (controllers.length > 1) {
                alias =
                    ' (' +
                    zimbra_notifier_popup._zimbra_notifier_Prefs.getUserAlias(
    ...
    if (unreadMessages.length === 0) {
        ...
        for (index = 0; index < unreadMessages.length && currentDisplayed < nbDisplayed; index++) {
            ...
            $('<div/>', {
                ...
                text: this._zimbra_notifier_Util.maxStringLength(
                    unreadMessages[index].content,
                    nbCharactersDisplayed,
                ),
            }).appendTo('#zimbra_mail_notifier_tooltipMessage');
            $('#zimbra_mail_notifier_tooltipMessage' + index).on('click', function () {
                zimbra_notifier_popup
                    .driver('openZimbraWebInterface', $(this).attr('accountId'))
                    .then(() => {
                        window.close();
                    });
            });
...
zimbra_notifier_popup.initializeTooltipCalendar = async function () {
    ...
    if (events.length === 0) {
        ...
        for (index = 0; index < events.length && currentDisplayed < nbDisplayed; index++) {
            ...
            var currentDate =
                chrome.i18n.getMessage('tooltip_week').replace('%WEEK%', currentEvent.startWeek) +
                ' - ' +
            ...
            if (currentEvent.duration < 86400000) {
                text =
                    ...
                    endTime +
                    '   ' +
                    this._zimbra_notifier_Util.maxStringLength(currentEvent.name, 40);
            } else {
