 ...
 * @this {Options}
 */
zimbra_notifier_options.init = function () {
    ...
    $('#zimbra_mail_notifier-emailNotificationFile').on('change', function (evt) {
        zimbra_notifier_Util.loadFile(
            evt.target.files[0],
            function (base64file) {
                zimbra_notifier_options.updatePref(
                    zimbra_notifier_Prefs.PREF.EMAIL_SOUND_FILE,
                    base64file,
                );
    ...
    $('#zimbra_mail_notifier-emailNotificationTestPlay').on('click', function () {
        ...
        var customSound =
            zimbra_notifier_options._emailSoundBase64File ||
            zimbra_notifier_Prefs.getEmailSoundCustom();
    ...
    $('#zimbra_mail_notifier-calendarNotificationFile').on('change', function (evt) {
        zimbra_notifier_Util.loadFile(
            ...
            function (base64file) {
                zimbra_notifier_options.updatePref(
                    zimbra_notifier_Prefs.PREF.CALENDAR_SOUND_FILE,
                    base64file,
                );
    ...
    $('#zimbra_mail_notifier-calendarNotificationTestPlay').on('click', function () {
        ...
        var customSound =
            zimbra_notifier_options._calendarSoundBase64File ||
            zimbra_notifier_Prefs.getCalendarSoundCustom();
        var volumeSound = parseInt(
            $('#zimbra_mail_notifier-optionCalendarNotificationVolume').val(),
        );
        zimbra_notifier_UiUtil.playSound(selected, customSound, volumeSound);
    });
...
zimbra_notifier_options.displayIdentifier = async function (controller) {
    ...
    $(
        '<fieldset id="identifier' +
            accountId +
            '" class="options identifier"> \
        <legend> \
            <label msg="option_identifiant_caption" class="section"></label> \
        </legend> \
    </fieldset>',
    ).insertBefore('#zimbra_mail_notifier-addNewIdentifier');
    ...
    $(
        ...
        <div class="subOptions"> \
            <div> \
                ...
                <input id="zimbra_mail_notifier-textboxAlias' +
            accountId +
            '" type="text" pref="userAlias' +
            accountId +
            '" style="width:160px;"></input> \
            </div> \
            <div> \
                <div msg="option_identifiant_login_label" msgtemplate="%MSG% : " style="width:200px;float:left;"></div> \
                <input id="zimbra_mail_notifier-textboxLogin' +
            accountId +
            '" type="text" pref="userLogin' +
            accountId +
            '" style="width:300px;"></input> \
            </div> \
            <div> \
                <div msg="option_identifiant_password_label" msgtemplate="%MSG% : " style="width:200px;float:left;"></div> \
                <input id="zimbra_mail_notifier-optionPassword' +
            accountId +
            '" type="password" pref="userPassword' +
            accountId +
            '" style="width:300px;"></input> \
            </div> \
            <div> \
                <input type="checkbox" id="zimbra_mail_notifier-checkboxSavePassword' +
            accountId +
            '" pref="userSavePassword' +
            accountId +
            '" style="margin-left:200px;"/> \
                <label for="zimbra_mail_notifier-checkboxSavePassword' +
            accountId +
            '" msg="option_identifiant_savePassword_label"></label> \
            </div> \
            <div> \
                <div msg="option_identifiant_2fatoken_label" msgtemplate="%MSG% : " style="width:200px;float:left;"></div> \
                <input id="zimbra_mail_notifier-option2faToken' +
            accountId +
            '" type="text" style="width:100px;margin-right:5px;"></input> \
                <button id="zimbra_mail_notifier-2faTokenButton' +
            accountId +
            '" msg="option_identifiant_2fatoken_button"></button> \
            </div> \
        </div> \
    </fieldset>',
    ).appendTo('#identifier' + accountId);
    ...
    $(
        ...
        <div class="subOptions"> \
            <div> \
                ...
                <input id="zimbra_mail_notifier-textboxUrlWebService' +
            accountId +
            '" type="text" pref="userServer' +
            accountId +
            '" msgtitle="option_identifiant_urlwebservice_tooltip" style="width:300px"></input> \
            </div> \
            <div> \
                <div msg="option_identifiant_urlwebinterface_label" msgtemplate="%MSG% : " style="width:200px;float:left;"></div><input id="zimbra_mail_notifier-textboxUrlWebInterface' +
            accountId +
            '" type="text" pref="userUrlWebInteface' +
            accountId +
            '" msgtitle="option_identifiant_urlwebinterface_tooltip" style="width:300px"></input> \
            </div> \
        </div> \
    </fieldset>',
    ).appendTo('#identifier' + accountId);
    ...
    $(
        '<div id="zimbra_mail_notifier-serverError' +
            accountId +
            '" class="zimbra_mail_notifier-serverError"></div>',
    ).appendTo('#identifier' + accountId);
    $(
        '<div class="actionButtons"> \
        <button id="zimbra_mail_notifier-removeButton' +
            accountId +
            '" msg="option_identifiant_remove_button"></button> \
        <button id="zimbra_mail_notifier-connectButton' +
            accountId +
            '" msg="option_identifiant_connect_button" style="float:right"></button> \
        <button id="zimbra_mail_notifier-connectCancelButton' +
            accountId +
            '" msg="option_identifiant_connectCancel_button" style="float:right"></button> \
        <button id="zimbra_mail_notifier-disconnectButton' +
            accountId +
            '" msg="option_identifiant_disconnect_button" style="float:right"></button> \
    </div>',
    ).appendTo('#identifier' + accountId);

    $('#zimbra_mail_notifier-removeButton' + accountId).on(
        'click',
        $.proxy(function () {
    ...
    $('#zimbra_mail_notifier-connectButton' + accountId).on(
        'click',
        $.proxy(function () {
    ...
    $('#zimbra_mail_notifier-connectCancelButton' + accountId).on(
        'click',
        $.proxy(function () {
    ...
    $('#zimbra_mail_notifier-disconnectButton' + accountId).on(
        'click',
        $.proxy(function () {
            this.disconnect(controller);
        }, this),
    ...
    $('#zimbra_mail_notifier-2faTokenButton' + accountId).on(
        'click',
        $.proxy(function () {
...
zimbra_notifier_options.showContent = function (contentId, animationTime) {
    if (
        !$.isNumeric(contentId) ||
        Math.floor(contentId) != contentId ||
        contentId < 0 ||
        contentId > 4
    ) {
        contentId = 0;
...
zimbra_notifier_options.refresh = async function (event, forced) {
    ...
    if (newIdentifierDisplayed) {
        ...
        $('*').each(function () {
            ...
            if (attr) {
                ...
                // add event on change
                if (
                    $(this).attr('id').indexOf('zimbra_mail_notifier-textboxLogin') >= 0 ||
                    $(this).attr('id').indexOf('zimbra_mail_notifier-optionPassword') >= 0 ||
                    $(this).attr('id').indexOf('zimbra_mail_notifier-textboxUrlWebService') >= 0
                ) {
                    // add event on keyup
    ...
    await Promise.all(
        controllers.map(async function (controller) {
            var accountId = controller.accountId;
            if (
                $('#zimbra_mail_notifier-textboxLogin' + accountId).val() !== '' &&
                $('#zimbra_mail_notifier-optionPassword' + accountId).val() !== '' &&
            ...
            if (controller.isConnected) {
                ...
                $('#zimbra_mail_notifier-textboxUrlWebService' + accountId).attr(
                    'disabled',
                    'disabled',
                );
