   ...
   if (event.command == "openPopup") {
        safari.extension.toolbarItems.forEach(function(item) {
            if (item.identifier === 'zimbraMailNotifierTab') {
                if(item.popover) {
                    if(item.popover.identifier !== 'ZimbraMailNotifierPopup') {
...
safari.application.addEventListener("message", function(event) {
    if (event.name == "zimbraMailNotifier_cookieResult") {
        chrome.cookies.callbackResult(event.message);
    }
},false);
    ...
    downloadLocale: function() {
        ...
        xhr.onreadystatechange = function() {
            if (this.readyState == 4) {
                zimbra_notifier_locale.locale =  JSON.parse(this.responseText) || {};
            }
        };
 ...
 */
zimbra_notifier_locale.downloadLocale();
